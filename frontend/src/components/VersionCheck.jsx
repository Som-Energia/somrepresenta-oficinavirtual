...
let obtainApiVersionTimeoutId = -1
...
async function obtainApiVersion() {
  ...
  clearTimeout(obtainApiVersionTimeoutId)
  obtainApiVersionTimeoutId = setTimeout(() => {
...
function VersionCheck({ children }) {
  ...
  onApiVersionUpdated = () => {
    ...
    console.error(
      `Version missmatch detected: ui ${uiVersion} !=== api ${apiVersion}`,
    )
    let time = new Date()
    ...
    setTimeout(
      () => forcedReload(),
      RELOAD_TIMEOUT_SECONDS * MILLISECONDS_IN_SECOND,
    )
    setSeconds(RELOAD_TIMEOUT_SECONDS)
    setInterval(
      () => setSeconds((s) => (s ? s - 1 : 0)),
  ...
  return (
    <>
      <Dialog open>
        <DialogTitle>
          {t("VERSION_CHECKER.NEW_VERSION_DETECTED_TITLE")}
        </DialogTitle>
        <DialogContent>
          <MuiMarkdown>
