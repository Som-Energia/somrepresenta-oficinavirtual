...
describe("euros formatting", () => {
  ...
  // NOTE: \xa0 is used instead space char because the formatter using this
  // This is the desired/correct behavior. Money units and cents should never be on to two different lines.
  // https://stackoverflow.com/a/54242671
  it("amount with no decimals", () => {
    expect(euros(2)).toBe("2,00\xa0€")
  })
  it("amount with two decimals", () => {
    expect(euros(2.15)).toBe("2,15\xa0€")
  })
  it("amount with rounding up", () => {
    expect(euros(2.159)).toBe("2,16\xa0€")
  })
  it("amount with rounding down", () => {
    expect(euros(2.151)).toBe("2,15\xa0€")
  })
  it("amount with rounding middle", () => {
    expect(euros(2.155)).toBe("2,16\xa0€")
  })
  ...
  it("amount over thousands puts point", () => {
    i18n.changeLanguage("es")
    expect(euros(2345)).toBe("2.345,00\xa0€")
  })
  it("basque locale", () => {
...
describe("percent formatting", () => {
  ...
  // NOTE: \xa0 is used instead space char because the formatter using this
  // This is the desired/correct behavior. Percent units and cents should never be on to two different lines.
  // https://stackoverflow.com/a/54242671
  it("amount with no decimals", () => {
    expect(percent(2)).toBe("2,00\xa0%")
  })
  it("amount with two decimals", () => {
    expect(percent(2.15)).toBe("2,15\xa0%")
  })
  it("amount with rounding up", () => {
    expect(percent(2.159)).toBe("2,16\xa0%")
  })
  it("amount with rounding down", () => {
    expect(percent(2.151)).toBe("2,15\xa0%")
  })
  it("amount with rounding middle", () => {
    expect(percent(2.155)).toBe("2,16\xa0%")
  })
  it("amount with negative", () => {
    expect(percent(-2)).toBe("-2,00\xa0%")
  })
  ...
  it("amount over thousands puts point", () => {
    i18n.changeLanguage("es")
    expect(percent(2345)).toBe("2.345,00\xa0%")
  })
  it("basque locale, prefixed percent sign", () => {
    i18n.changeLanguage("eu")
    expect(percent(2345)).toBe("%\xa02.345,00")
  })
  ...
  it("explicit decimals", () => {
    expect(percent(2.155, 0)).toBe("2\xa0%")
  })
})
...
describe("localISODateTime", () => {
  ...
  it("Zulu time resolves to local time", () => {
    expect(localISODateTime(new Date("2022-12-20 00:00:00Z"))).toBe(
      "2022-12-20 01:00:00",
    )
  })
  it("Local time resolves to same time", () => {
    expect(localISODateTime(new Date("2022-12-20 00:00:00"))).toBe(
      "2022-12-20 00:00:00",
    )
  })
})
